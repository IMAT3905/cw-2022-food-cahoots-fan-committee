...
namespace Engine
{
	std::string ImGuiHelper::s_iniFileLocation = std::string("../tool_imgui.ini");
	void ImGuiHelper::init()
	{
		...
		io.IniFilename = s_iniFileLocation.c_str();
	...
	void ImGuiHelper::end()
	{
		...
		ImGuiIO& io = ImGui::GetIO();
		auto& window = Engine::Application::getInstance().getWindow();
		io.DisplaySize = ImVec2(window->getWidthf(), window->getHeightf());
