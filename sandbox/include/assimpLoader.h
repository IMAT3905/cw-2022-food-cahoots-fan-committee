	...
	namespace Loader
	{
		...
		static void ASSIMPProcessMesh(aiMesh* mesh, const aiScene* scene)
		{
			...
			for (uint32_t i = 0; i < mesh->mNumVertices; i++)
			{
				...
				if (j != 0)Log::info("Bone IDs: {0}, {1}, {2}, {3} Weights: {4}, {5}, {6}, {7}", boneIds[0], boneIds[1], boneIds[2], boneIds[3], boneWeights[0], boneWeights[1], boneWeights[2], boneWeights[3]);*/
		...
		static void ASSIMPLoad(const std::string& filepath, uint32_t flags, std::shared_ptr<Material> material) //needs geometry
		{
